  ...
  for (int i=0 ; i<NUM ; i++) {
    ...
    v[i][1] = radius*((r[i][0]-14)/sqrt(length2));
    //v[i][0] = radius*(-((r[i][1]-10)));
    //v[i][1] = radius*((r[i][0]-14));
    //v[i][0] = radius*((r[i][0]-14));
    //v[i][1] = radius*((r[i][1]-10));
    v[i][2] = 0.0;

    printf("debug: %lf  %lf  %lf\n", sqrt(v[i][0]*v[i][0] +v[i][1]*v[i][1] +v[i][2]*v[i][2]),
    (-((r[i][1]-10)/sqrt(length2)))*(-((r[i][1]-10)/sqrt(length2)))
    +((r[i][0]-14)/sqrt(length2))*((r[i][0]-14)/sqrt(length2)),
        radius);
  }
